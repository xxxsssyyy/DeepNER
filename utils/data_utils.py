...
def input_from_line_padding(line, char_to_id, max_seq_len):
    """
    Take sentence data and return an input for
    the training or the evaluation function.
    """
    ...
    inputs.append([line])
    line.replace(" ", "$")
    char_ids = [char_to_id[char] if char in char_to_id else char_to_id["<UNK>"]
                   for char in line]
    seg_feas = get_seg_features(line)
    while len(char_ids) < max_seq_len:
        char_ids.append(0)
        seg_feas.append(0)
    inputs.append([char_ids])
    inputs.append([seg_feas])
    inputs.append([[]])
    return inputs
