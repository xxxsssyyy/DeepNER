...
from utils.utils import convert_single_example
...
def load_sentences(path, lower, zeros):
    ...
    for line in codecs.open(path, 'r', 'utf8'):
        ...
        # space is the hint of next line
        if not line:
            if len(sentence) > 0:
...
def bert_tag_mapping(sentences):
    """
    Create a dictionary and a mapping of tags, sorted by frequency.
    """
    tags = [[char[-1] for char in s] for s in sentences]
...
def prepare_padding_dataset(sentences, max_seq_length, char_to_id, tag_to_id, lower=False, train=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
    ...
    for s in sentences:
        ...
        if len(chars) >= max_seq_length:
            string = string[0:(max_seq_length)]
            chars = chars[0:(max_seq_length)]
            segs = segs[0:(max_seq_length)]
            tags = tags[0:(max_seq_length)]
        # padding
        while len(chars) < max_seq_length:
            string.append(0)
            chars.append(0)
            segs.append(0)
            tags.append(0)
...
def prepare_bert_dataset(sentences, max_seq_length, tag_to_id, lower=False, train=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
    ...
    for s in sentences:
        ...
        #chars = [char_to_id[f(w) if f(w) in char_to_id else '<UNK>']
        #         for w in string]
        char_line = ' '.join(string)   # join char with ' '
        ...
        labels = ' '.join(tags)     # join tags with ' '
        labels = tokenization.convert_to_unicode(labels)
        # fintuing mask with all 1 where char exists and padding ones with 0
...
def augment_with_pretrained(dictionary, ext_emb_path, chars):
    ...
    if chars is None:
        # add char in pretrained
        for char in pretrained:
            if char not in dictionary:
                dictionary[char] = 0
    else:
        # add char in test data
        for char in chars:
