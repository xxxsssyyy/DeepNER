...
def make_path(params):
    ...
    if not os.path.isdir(os.path.join(params.save_path, "log")):
        os.makedirs(os.path.join(params.save_path, "log"))
...
def create_bert_model(session, Model_class, path, config, logger):
    # create model, reuse parameters if exists
    model = Model_class(config)
...
def convert_single_example(char_line, tag_to_id, max_seq_length, tokenizer, label_line):
    """
    transform single line: char -> id, label -> tag_id
    """
