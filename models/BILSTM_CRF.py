...
class Model(object):
    def __init__(self, config):
        ...
        # attention layers
        # lstm_outputs = self.attention_layer(lstm_inputs, lstm_outputs)
    ...
    def biLSTM_layer(self, lstm_inputs, lstm_dim, lengths, name=None):
        ...
        :param lengths: [batch_size], the k-th value in tensor is the length of the k-th query length in a batch 
    ...
    def attention_layer(self, lstm_inputs, lstm_outputs, name=None):
        """
        ...
        with tf.variable_scope("attention" if not name else name):
            ...
            h1 = tf.layers.conv1d(lstm_inputs, 1, 1)
            h2 = tf.layers.conv1d(lstm_inputs, 1, 1)

            # [batch_size, num_steps, num_steps]
            coefs = h1 + tf.transpose(h2, [0, 2, 1])
            coefs = tf.nn.softmax(tf.nn.leaky_relu(coefs))
            
            # dropout

            # [batch_size, num_steps, num_steps] * [batch_size, num_steps, emb_size*2] = [batch_size, num_steps, emb_size*2]
            vals = tf.matmul(coefs, lstm_outputs)
            attention_out = tf.contrib.layers.bias_add(vals)
